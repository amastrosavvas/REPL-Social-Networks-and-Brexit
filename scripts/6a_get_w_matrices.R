...
nndat <-
  ...
  mutate( # calc inverse geographic distance / `commuting-discounted' measuress
    LCDi = 1/LCD, 
  ...
  mutate(
    across(c("SCI", "LCDi", "SCI_c", "LCDi_c"), ~dense_rank(desc(.)), .names = "n_{col}"),
    s1_5_SCI_c = ifelse(n_SCI_c %in% 1:5, 1, 0),  # define sets of neighbours
    s6_10_SCI_c = ifelse(n_SCI_c %in% 6:10, 1, 0),
    s1_10_SCI_c = ifelse(n_SCI_c %in% 1:10, 1, 0),
    s1_10_SCI = ifelse(n_SCI %in% 1:10, 1, 0),
    s1_5_LCDi_c = ifelse(n_LCDi_c %in% 1:5, 1, 0),
    s6_10_LCDi_c = ifelse(n_LCDi_c %in% 6:10, 1, 0),
    s1_10_LCDi_c = ifelse(n_LCDi_c %in% 1:10, 1, 0),
    s1_10_LCDi = ifelse(n_LCDi %in% 1:10, 1, 0),
    across( # Weights of neighbours within sets
      c("s1_5_SCI_c", "s6_10_SCI_c", contains("s1_10_SCI")),
      ~.*SCI/sum(.*SCI),
      .names = "{sub('s', 'w', col)}"
    ),
    across( 
      c("s1_5_LCDi_c", "s6_10_LCDi_c", contains("s1_10_LCD")),
      ~.*SCI/sum(.*SCI),
      .names = "{sub('s', 'w', col)}"
    ),
    across( # Equal weights within sets
      c(contains("s1_5_"), contains("s6_10")),
      ~./sum(.),
      .names = "{sub('s', 'weq', col)}"
    )
  ) %>% 
